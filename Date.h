...
// The parameters below takes designated values with the specified format semantics
	...
	const char* getDFormat();//to return formats and to avoid data leak
	const char* getMFormat();
	const char* getYFormat();
	
	void hideDFormat();//to not print date / month / year
	void hideMFormat();
	void hideYFormat();
...
public:
	...
	double operator-(const Date& otherDate);		// Number of days between otherDate and current date
	...
	friend bool validate(Day d,	Month m ,Year y);	
															//added friend function to check if date to be generated is valid
